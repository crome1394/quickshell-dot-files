...
Bar face shows the last IPv4 octet by default. Options → **Full IP on bar** (`showNetworkFullIp`) shows the complete address. Options → **Last octet on bar** (`showNetworkLastOctet`, default on) can hide that octet. Options → **Device name on bar** (`showNetworkDeviceName`) adds the adapter name (`enp10s0`, `wlan0`).
